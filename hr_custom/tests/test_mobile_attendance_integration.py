...
from hr_custom.api import mobile_attendance, portal_attendance
...
class TestMobileAttendanceIntegration(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        settings.update({"enable_mobile_attendance":1,"require_geolocation":1,"require_branch":1,"allow_without_branch":0,"require_registered_device":0})
    ...
    def test_07_missing_coordinates_rejected(self):
        with self.assertRaises(frappe.ValidationError): mobile_attendance.submit_checkin(None,None,None)
    ...
    def test_18_hrms_owns_shift_and_attendance_fields(self):
        source = mobile_attendance.submit_checkin.__wrapped__.__code__.co_names
        self.assertNotIn("timing_flags", source)
        self.assertNotIn("resolve_shift", source)
    def test_19_status_exposes_fast_location_and_portal_settings(self):
        result = mobile_attendance.get_status()
        self.assertGreaterEqual(result["location_cache_seconds"], 0)
        self.assertGreater(result["maximum_gps_accuracy"], 0)
        self.assertIn("leaves", result["portal_tabs"])
    def test_19b_attendance_history_is_employee_scoped(self):
        created = mobile_attendance.submit_checkin(24.7136,46.6753,5)
        history = mobile_attendance.get_attendance_history(5)
        self.assertEqual(history[0].name, created["name"])
        self.assertTrue(all(row.name != "" for row in history))
    ...
    def test_24_missing_checkout_exception_idempotent(self):
        yesterday=datetime.combine(getdate(frappe.utils.add_days(nowdate(),-1)),datetime.min.time())+timedelta(hours=8)
    ...
    def test_27_attendance_period_is_employee_scoped(self):
        name = frappe.db.get_value("Attendance", {"employee": self.employee, "attendance_date": nowdate()}, "name")
        if not name:
            frappe.get_doc({"doctype":"Attendance","employee":self.employee,"attendance_date":nowdate(),"status":"Present","company":"_Test Company"}).insert(ignore_permissions=True).submit()
        elif frappe.db.get_value("Attendance", name, "docstatus") == 0:
            attendance = frappe.get_doc("Attendance", name)
            attendance.flags.ignore_permissions = True
            attendance.submit()
        rows = mobile_attendance.get_attendance_period(nowdate(), nowdate())
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0].employee, self.employee)

    def test_27b_attendance_period_sorts_official_and_pending_rows(self):
        yesterday = frappe.utils.add_days(nowdate(), -1)
        official = frappe.db.get_value("Attendance", {"employee": self.employee, "attendance_date": yesterday}, "name")
        if not official:
            frappe.get_doc({"doctype": "Attendance", "employee": self.employee, "attendance_date": yesterday, "status": "Present", "company": "_Test Company"}).insert(ignore_permissions=True).submit()
        pending_time = datetime.combine(getdate(nowdate()), datetime.min.time()) + timedelta(hours=8)
        frappe.get_doc({"doctype": "Employee Checkin", "employee": self.employee, "time": pending_time, "log_type": "IN"}).insert(ignore_permissions=True)
        rows = portal_attendance.get_attendance_period(yesterday, nowdate())
        self.assertEqual([getdate(row.get("attendance_date")) for row in rows], [getdate(nowdate()), getdate(yesterday)])
        self.assertTrue(rows[0].get("is_pending"))

    def test_28_period_rejects_more_than_one_year(self):
        with self.assertRaises(frappe.ValidationError):
            mobile_attendance.get_attendance_period("2024-01-01", "2025-01-02")

    def test_29_profile_is_current_employee(self):
        self.assertEqual(mobile_attendance.get_profile_data().name, self.employee)

    def test_30_settings_tab_is_not_exposed(self):
        self.assertNotIn("settings", mobile_attendance.get_status()["portal_tabs"])

    def test_31_portal_notifications_are_user_scoped(self):
        notification = frappe.get_doc({"doctype": "PWA Notification", "from_user": "Administrator", "to_user": self.user, "message": "Test HR update", "reference_document_type": "Salary Slip", "reference_document_name": "TEST-SLIP"})
        notification.insert(ignore_permissions=True)
        result = mobile_attendance.get_portal_notifications()
        self.assertTrue(any(row.name == notification.name for row in result["items"]))
        self.assertGreaterEqual(result["unread"], 1)

    def test_32_mark_notifications_read_is_user_scoped(self):
        notification = frappe.get_doc({"doctype": "PWA Notification", "from_user": "Administrator", "to_user": self.user, "message": "Read me", "reference_document_type": "Leave Application", "reference_document_name": "TEST-LEAVE"})
        notification.insert(ignore_permissions=True)
        mobile_attendance.mark_portal_notifications_read()
        self.assertEqual(frappe.db.get_value("PWA Notification", notification.name, "read"), 1)

    def test_33_mark_single_notification_read(self):
        notification = frappe.get_doc({"doctype": "PWA Notification", "from_user": "Administrator", "to_user": self.user, "message": "Announcement title\nAnnouncement message", "reference_document_type": "HR Announcement", "reference_document_name": "TEST-ANNOUNCEMENT"})
        notification.insert(ignore_permissions=True)
        mobile_attendance.mark_portal_notification_read(notification.name)
        self.assertEqual(frappe.db.get_value("PWA Notification", notification.name, "read"), 1)

    def test_34_archive_and_delete_notifications_are_user_scoped(self):
        archived = frappe.get_doc({"doctype": "PWA Notification", "from_user": "Administrator", "to_user": self.user, "message": "Archive me", "reference_document_type": "Salary Slip", "reference_document_name": "TEST-ARCHIVE"}).insert(ignore_permissions=True)
        mobile_attendance.archive_portal_notification(archived.name)
        self.assertEqual(frappe.db.get_value("PWA Notification", archived.name, "custom_archived"), 1)
        self.assertFalse(any(row.name == archived.name for row in mobile_attendance.get_portal_notifications()["items"]))
        self.assertTrue(any(row.name == archived.name for row in mobile_attendance.get_portal_notifications(include_archived=1)["items"]))
        mobile_attendance.unarchive_portal_notification(archived.name)
        self.assertEqual(frappe.db.get_value("PWA Notification", archived.name, "custom_archived"), 0)

        deleted = frappe.get_doc({"doctype": "PWA Notification", "from_user": "Administrator", "to_user": self.user, "message": "Delete me", "reference_document_type": "Salary Slip", "reference_document_name": "TEST-DELETE"}).insert(ignore_permissions=True)
        mobile_attendance.delete_portal_notification(deleted.name)
        self.assertFalse(frappe.db.exists("PWA Notification", deleted.name))
