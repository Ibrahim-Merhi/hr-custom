frappe.ready(() => {
	...
	byId("logout-button").addEventListener("click", async (event) => {
		...
		if (!confirmed) return;
		try { await fetch("/api/method/hr_custom.api.portal_auth.logout", {method: "POST", credentials: "same-origin", headers: {"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}}); } finally { localStorage.removeItem("hr_attendance_logged_in"); location.replace(`/attendance?logout=${Date.now()}`); }
	});
