frappe.ready(() => {
  ...
  form.addEventListener("submit",async event=>{event.preventDefault();event.stopImmediatePropagation();button.disabled=true;button.textContent=__("Signing in…");loginMessage.textContent="";const body=new URLSearchParams({usr:byId("login-email").value.trim(),pwd:byId("login-password").value});try{const response=await fetch("/api/method/login",{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8","Accept":"application/json"},body});const data=await response.json();if(!response.ok||!(data.message==="Logged In"||data.message==="No App"))throw new Error(__("Invalid username or password."));localStorage.setItem("hr_attendance_logged_in","1");location.replace(`/attendance?login=${Date.now()}`);}catch(error){button.disabled=false;button.textContent=__("Sign In");loginMessage.textContent=error.message||__("Invalid username or password.");loginMessage.className="feedback error";}},true);
