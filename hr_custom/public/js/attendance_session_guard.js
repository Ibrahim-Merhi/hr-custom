frappe.ready(() => {
  ...
  function isPortalRequest(settings){return String(settings?.url||"").includes("/api/method/hr_custom.");}
  ...
  // Only failed hr_custom requests can invalidate the employee portal session.
  window.jQuery(document).ajaxError((event,xhr,settings)=>{if(isPortalRequest(settings)&&isSessionError(xhr))showLogin();});
